...
function ReviewCard({ name, date, text, rating }) {
  return (
    <div className="w-[270px] rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          <p className="text-[15px] font-semibold leading-tight text-slate-800">{name}</p>
          <p className="truncate text-xs leading-tight text-slate-400">{date}</p>
        </div>
      </div>
      <div className="mt-3">
        <Stars n={rating} />
      </div>
      <p className="mt-2.5 line-clamp-6 text-[13px] leading-relaxed text-slate-600">{text}</p>
    </div>
  );
}
 ...
 * из бегущих колонок. Колонки раздаются по кругу, чтобы карточки в соседних
...
export default function Reviews() {
  ...
  return (
    <section aria-labelledby="reviews-title" className="overflow-hidden bg-slate-50 py-16 md:py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-10 flex flex-wrap items-end justify-between gap-6 md:mb-14">
          <div>
            <h2 id="reviews-title" className="section-title !text-left">Отзывы пациентов</h2>
            <p className="section-subtitle !text-left !mx-0">Что о нас говорят на Яндекс Картах</p>
          </div>
          <YandexRating />
        </div>
        ...
        <div className="relative flex h-[460px] items-center justify-center [perspective:1200px]">
          <div
            ...
            style={{ transform: 'rotateX(5deg) rotateY(-11deg) rotateZ(4deg) scale(1.04)' }}
          >
