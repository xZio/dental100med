...
router.post('/', adminOnly, (req, res) => {
  try {
    const { title, description, discount, link, active, expiresAt } = req.body;
    ...
    const { lastInsertRowid } = db
      .prepare(`INSERT INTO promotions (title, description, discount, link, active, expiresAt, createdAt, updatedAt)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(title.trim(), description || '', discount || '', cleanLink(link), active === false ? 0 : 1, toISO(expiresAt), ts, ts);
